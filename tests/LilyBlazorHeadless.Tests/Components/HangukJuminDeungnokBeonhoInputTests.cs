...
public class HangukJuminDeungnokBeonhoInputTests : TestContext
{
    [Fact]
    public void Renders_as_text_input_with_kebab_base_class()
    {
        var cut = RenderComponent<HangukJuminDeungnokBeonhoInput>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".hanguk-jumin-deungnok-beonho-input");
    ...
    {
        var cut = RenderComponent<HangukJuminDeungnokBeonhoInput>(p => p
            .Add(x => x.Label, "Label")
