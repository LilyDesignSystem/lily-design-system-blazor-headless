...
public class CanadaSocialInsuranceNumberViewTests : TestContext
{
    [Fact]
    public void Renders_as_span_with_kebab_base_class()
    {
        var cut = RenderComponent<CanadaSocialInsuranceNumberView>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".canada-social-insurance-number-view");
    ...
    {
        var cut = RenderComponent<CanadaSocialInsuranceNumberView>(p => p
            .Add(x => x.Label, "Label")
