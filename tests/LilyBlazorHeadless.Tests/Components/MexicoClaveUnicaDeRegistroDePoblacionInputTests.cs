...
public class MexicoClaveUnicaDeRegistroDePoblacionInputTests : TestContext
{
    [Fact]
    public void Renders_as_text_input_with_kebab_base_class()
    {
        var cut = RenderComponent<MexicoClaveUnicaDeRegistroDePoblacionInput>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".mexico-clave-unica-de-registro-de-poblacion-input");
    ...
    {
        var cut = RenderComponent<MexicoClaveUnicaDeRegistroDePoblacionInput>(p => p
            .Add(x => x.Label, "Label")
