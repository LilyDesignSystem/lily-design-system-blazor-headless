...
public class ChileRolUnicoNacionalInputTests : TestContext
{
    [Fact]
    public void Renders_as_text_input_with_kebab_base_class()
    {
        var cut = RenderComponent<ChileRolUnicoNacionalInput>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".chile-rol-unico-nacional-input");
    ...
    {
        var cut = RenderComponent<ChileRolUnicoNacionalInput>(p => p
            .Add(x => x.Label, "Label")
