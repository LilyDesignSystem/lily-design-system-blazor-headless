...
public class NihonKojinBangoViewTests : TestContext
{
    [Fact]
    public void Renders_as_span_with_kebab_base_class()
    {
        var cut = RenderComponent<NihonKojinBangoView>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".nihon-kojin-bango-view");
    ...
    {
        var cut = RenderComponent<NihonKojinBangoView>(p => p
            .Add(x => x.Label, "Label")
