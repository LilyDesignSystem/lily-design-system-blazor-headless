...
public class MisrAlRaqmAlQawmiInputTests : TestContext
{
    [Fact]
    public void Renders_as_text_input_with_kebab_base_class()
    {
        var cut = RenderComponent<MisrAlRaqmAlQawmiInput>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".misr-al-raqm-al-qawmi-input");
    ...
    {
        var cut = RenderComponent<MisrAlRaqmAlQawmiInput>(p => p
            .Add(x => x.Label, "Label")
