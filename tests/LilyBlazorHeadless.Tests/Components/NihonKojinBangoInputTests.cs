...
public class NihonKojinBangoInputTests : TestContext
{
    [Fact]
    public void Renders_as_text_input_with_kebab_base_class()
    {
        var cut = RenderComponent<NihonKojinBangoInput>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".nihon-kojin-bango-input");
    ...
    {
        var cut = RenderComponent<NihonKojinBangoInput>(p => p
            .Add(x => x.Label, "Label")
