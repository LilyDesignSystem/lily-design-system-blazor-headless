...
public class PilipinasPhilhealthIdentificationNumberInputTests : TestContext
{
    [Fact]
    public void Renders_as_text_input_with_kebab_base_class()
    {
        var cut = RenderComponent<PilipinasPhilhealthIdentificationNumberInput>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".pilipinas-philhealth-identification-number-input");
    ...
    {
        var cut = RenderComponent<PilipinasPhilhealthIdentificationNumberInput>(p => p
            .Add(x => x.Label, "Label")
