...
public class MisrAlRaqmAlQawmiViewTests : TestContext
{
    [Fact]
    public void Renders_as_span_with_kebab_base_class()
    {
        var cut = RenderComponent<MisrAlRaqmAlQawmiView>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".misr-al-raqm-al-qawmi-view");
    ...
    {
        var cut = RenderComponent<MisrAlRaqmAlQawmiView>(p => p
            .Add(x => x.Label, "Label")
