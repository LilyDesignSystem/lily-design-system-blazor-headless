...
public class RossiyaSnilsViewTests : TestContext
{
    [Fact]
    public void Renders_as_span_with_kebab_base_class()
    {
        var cut = RenderComponent<RossiyaSnilsView>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".rossiya-snils-view");
    ...
    {
        var cut = RenderComponent<RossiyaSnilsView>(p => p
            .Add(x => x.Label, "Label")
