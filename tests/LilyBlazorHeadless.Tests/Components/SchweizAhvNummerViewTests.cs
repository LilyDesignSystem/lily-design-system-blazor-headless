...
public class SchweizAhvNummerViewTests : TestContext
{
    [Fact]
    public void Renders_as_span_with_kebab_base_class()
    {
        var cut = RenderComponent<SchweizAhvNummerView>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".schweiz-ahv-nummer-view");
    ...
    {
        var cut = RenderComponent<SchweizAhvNummerView>(p => p
            .Add(x => x.Label, "Label")
