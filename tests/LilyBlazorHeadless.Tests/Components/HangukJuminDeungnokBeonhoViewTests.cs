...
public class HangukJuminDeungnokBeonhoViewTests : TestContext
{
    [Fact]
    public void Renders_as_span_with_kebab_base_class()
    {
        var cut = RenderComponent<HangukJuminDeungnokBeonhoView>(p => p.Add(x => x.Label, "Label"));
        var root = cut.Find(".hanguk-jumin-deungnok-beonho-view");
    ...
    {
        var cut = RenderComponent<HangukJuminDeungnokBeonhoView>(p => p
            .Add(x => x.Label, "Label")
